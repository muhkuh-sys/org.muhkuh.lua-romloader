...
docker exec ${ID} bash -c 'tar --create --file /tmp/work/build/build_lua5.1_ubuntu_1404_x86.tar.gz --gzip --directory /tmp/work/build/linux32/lua5.1/install .'
docker exec ${ID} bash -c 'tar --create --file /tmp/work/build/build_lua5.2_ubuntu_1404_x86.tar.gz --gzip --directory /tmp/work/build/linux32/lua5.2/install .'
docker exec ${ID} bash -c 'tar --create --file /tmp/work/build/build_lua5.3_ubuntu_1404_x86.tar.gz --gzip --directory /tmp/work/build/linux32/lua5.3/install .'
