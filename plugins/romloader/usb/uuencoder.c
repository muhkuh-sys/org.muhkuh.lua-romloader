...
size_t uuencoder::process(unsigned char *pucLine, size_t sizMaxLine)
{
	size_t sizLine;
	size_t sizChunk;
	const unsigned char *pucInputCnt;
	unsigned char *pucOutputCnt;
	...
	case UUENCODE_STATE_Begin:
		sizLine = snprintf(pucLine, sizMaxLine, "begin 666 -\n");
	...
	case UUENCODE_STATE_Data:
		...
		pucOutputCnt = pucLine;

		/* Print the length character for the line. */
		*(pucOutputCnt++) = (unsigned char)(0x20 + sizChunk);
		...
		do
		{
			...
			do
			{
				*(pucOutputCnt++) = (unsigned char)(0x20 + (ulUueBuf>>26));
				ulUueBuf <<= 6;
			} while( --iCnt!=0 );
		} while( sizChunk!=0 );
	...
	case UUENCODE_STATE_Last1:
		sizLine = snprintf(pucLine, sizMaxLine, "`\n");
		m_tState = UUENCODE_STATE_Last2;
		break;

	case UUENCODE_STATE_Last2:
		sizLine = snprintf(pucLine, sizMaxLine, "end\n");
