...
bool romloader_uart_options::getOption_netx90MiImage(const char **ppcImageData, size_t *psizImageLen)
{
				...
				case ROMLOADER_COMMANDSET_ABOOT_OR_HBOOT1:
					...
					if( fResult!=true )
					{
						...
						if( fResult!=true )
						{
							...
							/* If that did not work, try again, expecting a HBOOT3 prompt (includes a status code). */
							ptFn = &tFnHBoot3;
							fResult = detect_chiptyp(ptFn);
							if( fResult!=true )
							{
