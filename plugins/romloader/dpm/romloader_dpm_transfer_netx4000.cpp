...
romloader_dpm_transfer_netx4000::romloader_dpm_transfer_netx4000(romloader_dpm_device *ptDpmDevice)
...
int romloader_dpm_transfer_netx4000::prepare_device(void)
{
	...
	} else
	{
		...
		iResult = m_ptDpmDevice->dpm_write32(offsetof(DPM_REGISTER_NETX4000_RELAXED_T,tCtrl)+offsetof(NX4000_DPM_AREA_T, ulDpm_rdy_cfg), ulValue);
		if( iResult==0 )
		{
			/* Disable setup times and filter. */
			ulValue = 0;
			iResult = m_ptDpmDevice->dpm_write32(offsetof(DPM_REGISTER_NETX4000_RELAXED_T, tCtrl)+offsetof(NX4000_DPM_AREA_T, ulDpm_timing_cfg), ulValue);
			if( iResult==0)
			{
				...
				case NETX_DEVICE_STATE_Romloader:
					...
					iResult = download_and_run_image(auc_dpmmon_netx4000_monitor,sizeof(auc_dpmmon_netx4000_monitor));
...
int romloader_dpm_transfer_netx4000::send_command(const uint8_t *pucCommand, uint32_t ulCommandSize)
{
	int iResult;
 ...
 */
int romloader_dpm_transfer_netx4000::receive_response(uint8_t *pucBuffer, uint32_t ulDataSize)
{
	int iResult;
...
int romloader_dpm_transfer_netx4000::receive_packet(uint8_t *pucBuffer,	uint32_t ulMaxDataSize, uint32_t *pulDataRead)
{
	int iResult;
...
romloader_dpm_transfer_netx4000::NETX_DEVICE_STATE_T romloader_dpm_transfer_netx4000::probe_netx_state(void)
{
	NETX_DEVICE_STATE_T tState;
	int iResult;
	uint32_t ulValue;
	...
	if( m_ptDpmDevice!=NULL )
	{
		...
		iResult = m_ptDpmDevice->dpm_read32(offsetof(DPM_REGISTER_NETX4000_RELAXED_T, tHboot)+offsetof(HBOOT_DPM_NETX4000_RELAXED_T, ulDpmBootId), &ulValue);
		if( iResult==0 )
		{
...
int romloader_dpm_transfer_netx4000::mailbox_purr(uint32_t ulMask, uint32_t *ulResult)
{
	int iResult;
	uint32_t ulValue;
	uint32_t ulHostPart;
...
int romloader_dpm_transfer_netx4000::mailbox_send_chunk(const uint8_t *pucChunk, uint32_t ulChunkSize)
{
	uint32_t ulValue;
	unsigned int uiRetryCnt;
	int iResult;
	...
	if( iResult==0 )
	{
		...
		if( iResult==0 )
		{
			...
			{
				printf("4\n");
				ulValue ^= NETX4000_DPM_BOOT_HOST_SEND_CMD << SRT_NETX4000_HANDSHAKE_REG_PC_DATA;
				iResult = m_ptDpmDevice->dpm_write32(OFFS_HBOOT(ulHandshake), ulValue);
				if( iResult==0 )
				{
					...
					{
						printf("5\n");
						iResult = mailbox_purr(NETX4000_DPM_BOOT_NETX_RECEIVED_CMD, &ulValue);
						if( iResult!=0 )
						{
							break;
						}
...
int romloader_dpm_transfer_netx4000::download_and_run_image(const uint8_t *pucData, size_t sizData)
{
	int iResult;
				...
				else
				{
					...
					if( ucStatus==0 )
					{
						while( pucCnt<pucEnd )
						{
							...
							{
								ulChunkSize = NETX4000_RELAXED_DPM_HOST_TO_NETX_BUFFERSIZE;
							}
