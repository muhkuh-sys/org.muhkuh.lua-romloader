...
romloader_dpm_transfer_netx56::romloader_dpm_transfer_netx56(
...
int romloader_dpm_transfer_netx56::prepare_device(void) {
	...
	if (m_ptDpmDevice == NULL) {
		...
		iResult = m_ptDpmDevice->dpm_write32(
				offsetof(DPM_REGISTER_NETX56_T, tCtrl)
						+ offsetof(NX56_DPM_AREA_T, ulDpm_rdy_cfg), ulValue);
		if (iResult == 0) {
			/* Disable setup times and filter. */
			ulValue = 0;
			iResult = m_ptDpmDevice->dpm_write32(
					offsetof(DPM_REGISTER_NETX56_T, tCtrl)
							+ offsetof(NX56_DPM_AREA_T, ulDpm_timing_cfg),
					ulValue);
			if (iResult == 0) {
				...
				case NETX_DEVICE_STATE_Romloader:
					...
					iResult = download_and_run_image(auc_dpmmon_netx56_monitor,
							sizeof(auc_dpmmon_netx56_monitor));
...
int romloader_dpm_transfer_netx56::send_command(const uint8_t *pucCommand,
 ...
 */
int romloader_dpm_transfer_netx56::receive_response(uint8_t *pucBuffer,
		uint32_t ulDataSize) {
	int iResult;
...
int romloader_dpm_transfer_netx56::receive_packet(uint8_t *pucBuffer,
 ...
 *
 * */
int romloader_dpm_transfer_netx56::receive_escape_acknowledge_command() {
	uint32_t ulValue;
...
romloader_dpm_transfer_netx56::NETX_DEVICE_STATE_T romloader_dpm_transfer_netx56::probe_netx_state(
		void) {
	NETX_DEVICE_STATE_T tState;
	int iResult;
	uint32_t ulValue;
	...
	if (m_ptDpmDevice != NULL) {
		...
		iResult = m_ptDpmDevice->dpm_read32(
				offsetof(DPM_REGISTER_NETX56_T, tHboot)
						+ offsetof(HBOOT_DPM_NETX56_T, ulDpmBootId), &ulValue);
		if (iResult == 0) {
...
int romloader_dpm_transfer_netx56::mailbox_purr(uint32_t ulMask,
		uint32_t *ulResult) {
	int iResult;
	uint32_t ulValue;
	uint32_t ulHostPart;
...
int romloader_dpm_transfer_netx56::mailbox_send_chunk(const uint8_t *pucChunk,
	...
	if (iResult == 0) {
		...
		if (iResult == 0) {
			...
			if (iResult == 0) {
				printf("4\n");
				ulValue ^= NETX56_DPM_BOOT_HOST_SEND_CMD
						<< SRT_NETX56_HANDSHAKE_REG_PC_DATA;
				iResult = m_ptDpmDevice->dpm_write32(OFFS_HBOOT(ulHandshake),
						ulValue);
				if (iResult == 0) {
					...
					do {
						...
						NETX56_DPM_BOOT_NETX_RECEIVED_CMD, &ulValue);
						if (iResult != 0) {
							break;
						} else if (ulValue != 0) {
							--uiRetryCnt;
...
//	printf("Retries left: %d\n", uiRetryCnt);
	...
	if (iResult == 0) {
		iResult = m_ptDpmDevice->dpm_read32(OFFS_HBOOT(ulNetxToHostDataSize),
				&ulRec);
		if (iResult == 0) {
			if (ulRec > NETX56_DPM_NETX_TO_HOST_BUFFERSIZE) {
				...
				iResult = m_ptDpmDevice->dpm_read_area(
						OFFS_HBOOT(aucNetxToHostData), m_aucBufferNetxToHost,
						ulRec);
				if (iResult == 0) {
					/* Toggle the handshake bit. */
					iResult = m_ptDpmDevice->dpm_read32(OFFS_HBOOT(ulHandshake),
							&ulValue);
					if (iResult == 0) {
						ulValue ^= NETX56_DPM_BOOT_HOST_RECEIVED_CMD
								<< SRT_NETX56_HANDSHAKE_REG_PC_DATA;
						iResult = m_ptDpmDevice->dpm_write32(
								OFFS_HBOOT(ulHandshake), ulValue);
						if (iResult == 0) {
							*pulSize = ulRec;
						}
					}
...
int romloader_dpm_transfer_netx56::download_and_run_image(
					...
					if (ucStatus == 0) {
						while (pucCnt < pucEnd) {
							...
							if (ulChunkSize > NETX56_DPM_HOST_TO_NETX_BUFFERSIZE) {
								ulChunkSize =
								NETX56_DPM_HOST_TO_NETX_BUFFERSIZE;
							}
