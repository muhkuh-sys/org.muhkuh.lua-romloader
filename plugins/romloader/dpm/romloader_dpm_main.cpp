...
void romloader_dpm::call(uint32_t ulNetxAddress, uint32_t ulParameterR0, SWIGLUA_REF tLuaFn, long lCallbackUserData)
{
	...
	else
	{
		...
		aucCommand[0x00] = MONITOR_COMMAND_Execute | (uint8_t)(m_uiMonitorSequence << MONITOR_SEQUENCE_SRT);
		...
		else
		{
			...
			else
			{
				...
				while( 1 )
				{
					...
					uint8_t aucLocalResponse[m_sizMaxPacketSizeClient]; // here we can't predict what netX is going to send us so we take m_sizMaxPacketSizeClient
					uint32_t ulPacketSize;

					iResult = m_ptTransfer->receive_packet(aucLocalResponse, m_sizMaxPacketSizeClient, &ulPacketSize);
//					aucLocalResponse[ulPacketSize] = 0;
					...
					if( pcProgressData!=NULL)
					{
						...
						if( fIsRunning!=true )
						{
							...
							printf("send done: %d\n", iResult);
							printf("Receive_packet. \n");
							iResult = m_ptTransfer->receive_escape_acknowledge_command();
