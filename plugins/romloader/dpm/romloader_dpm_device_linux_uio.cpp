...
romloader_dpm_device_linux_uio::romloader_dpm_device_linux_uio(unsigned int uiUioDevice) :
		m_uiUioDevice(uiUioDevice), m_uiUioBar(0), m_pcUioName(NULL), m_ulUioAddress(0), m_sizUioMemory(0)
{
	m_pvUioMappedMemory.pv = NULL;
}

romloader_dpm_device_linux_uio::romloader_dpm_device_linux_uio(const char *pcDeviceName) :
		m_uiUioDevice(0xFFFFFFFF), m_uiUioBar(0), m_pcUioName(NULL), m_ulUioAddress(0), m_sizUioMemory(0)
{
	int iScanResult;
...
void copy_loop(void * dst, void * src, uint32_t len)
{
	for (int i = 0; i < len; i++)
	{
...
int romloader_dpm_device_linux_uio::dpm_read_area(uint32_t ulDpmAddress, uint8_t *pucData, size_t sizData)
{
	...
	{
		/**
		 * @todo memcpy seems to be not working --> further investigations here
		 */
//		memcpy(pucData, m_pvUioMappedMemory.puc+ulDpmAddress, sizData);
		copy_loop(pucData, m_pvUioMappedMemory.puc + ulDpmAddress, sizData);
		iResult = 0;
	}
...
{
{ .pcName = "netx_plx", .uiBar = 0, .ulBarSize = 16384 } };
